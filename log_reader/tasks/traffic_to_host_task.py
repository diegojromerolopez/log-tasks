...
class TrafficToHostTask(object):
    ...
    def run(self):
        ...
        def hosts_connected_to_desired_host_extractor(log_line):
            # Assuming the file is ordered by timestamp (as any log file should be)
            # if the log line is has a greater date than the end timestamp,
            # that line (and the next ones) must not be read. Our read is complete.
            if log_line.unix_timestamp > self.end_timestamp:
            ...
            if (
                    self.start_timestamp <= log_line.unix_timestamp and
